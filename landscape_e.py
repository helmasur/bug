...
def main():
    ...
    def topwave(x):
        return 1-sin(acos(x*2%2-1))
    

    for x in range(400):
        plot.set(x, 15 - sawwave(x/50.0)*10, 0) # gör x till alltid float så man kan / 20
        plot.set(x, 30 - sinwave(x/50.0)*10, 0)
        plot.set(x, 50 - sqrwave(x/50.0)*10,0)
        plot.set(x, 70 - triwave(x/50.0)*10, 0)
        plot.set(x, 100 - bllwave(x/50.0)*25, 0)
        plot.set(x, 125 - topwave(x/50.0)*25, 0)
    plot.set(50,0,0)
