...
class Player(pygame.sprite.Sprite):
    "stores information about the player, and other data"
    def __init__(self):
...
class Groundsprite(pygame.sprite.Sprite):
    def __init__(self, pos, image, WCview, spritesize, entry):
        pygame.sprite.Sprite.__init__(self) #call Sprite initializer
    ...
    def update(self, player):
        ...
        self.rect.bottomleft = (self.origin[0] - player.WC[0], self.origin[1] + player.WC[1])
...
class World_2000():
    def __init__(self, player):
        ...
        self.size = (2,2)
        self.entry = (100,100)
        self.spritesize = 2000.0
        self.image = load_image('2000px.tga')
        self.sprite = Groundsprite((0,0), self.image, player.WCview, self.spritesize, self.entry)
    ...
    def render(self, target):
        self.sprite_group.draw(target)

class Groundsprite_infineon(Groundsprite):
...
class World_infineon(): #this world is coded here
    def __init__(self, screensize, WCview):
        self.name="Infineon"
        self.size=(0,0)
        self.spritesize = 64.0 # float needed
        self.imagebank = dict()
        ...
        amountgroundspritesx = int(math.ceil(screensize[0]/self.spritesize)+1)
        amountgroundspritesy = int(math.ceil(screensize[1]/self.spritesize)+1)
        ...
        for y in range(amountgroundspritesy):
            for x in range(amountgroundspritesx):
                self.groundspritelist.append(Groundsprite_infineon((x, y), self.imagebank, WCview, int(self.spritesize)))
                self.groundspritegroup.add(self.groundspritelist[len(self.groundspritelist)-1])

    def update(self, player):
        self.groundspritegroup.update(player, self.imagebank)
...
#Prepare Game Objects
    ...
    player = Player()
    player.screensize = screen.get_size()
    enemy = Enemy()
